...
async def _run_auto_analysis(new_rows: list[dict]):
    ...
    user_message = (
        ...
        f"[출력 구조 — 헤더명·순서 고정]\n"
        f"응답 맨 위 헤드라인 1줄 → `## 오늘 체성분` → `## 오늘 추천` → "
        f"`## 30일 훈련 볼륨` → `## 러닝 강도 분포` → `## 수면`. "
        f"헤더명 임의 변경 금지.\n\n"
        f"[지시]\n"
        f"1. **헤드라인 필수 (맨 위 한 줄)**: `📍 {latest_date} — 다이어트 X주차 누적 -Ykg (107→현재)` 형식. "
        f"주차·누적 감량은 메모리(§0 프로필: 시작 107kg)와 오늘 체중으로 계산. "
        f"헤드라인 없으면 오너가 오늘 수치를 고립 해석하게 되므로 반드시 포함.\n"
        f"2. **표 간소화**: `## 오늘 체성분` 표는 **오늘값 + 방향(↑↓→) + 신호등(🟢 밴드 내 / 🟡 경계 / 🔴 이탈)만** 표기. "
        f"밴드 min/max 숫자 노출 금지(인지부하 최소화). 표 아래 해석은 2-3줄로 압축.\n"
        f"3. **라벨 규칙 + 밴드 산출 (source 필터 강제)**: "
        f"`muscle_mass_kg` 컬럼은 `source=apple_health`면 **제지방(FFM)**, "
        f"`source=inbody`(가정용·전문가 장비 모두) / `source=manual`이면 **골격근**. "
        f"자동 수집(이 트리거)은 apple_health → 제지방으로 다룰 것. "
        f"밴드 산출 시 `mcp__body_metrics__get_body_metrics_history`로 최근 60일 조회 후 "
        f"**반드시 `source=apple_health` 행만 필터링**해서 밴드(min/max)를 낼 것 "
        f"(inbody 41kg 스케일 섞이면 밴드 30kg 폭으로 무의미). "
        f"밴드 내면 '60일 정상 변동 범위 내 — 다이어트 궤도 유지'로 프레이밍('정체' 반복 금지 — 학습된 무기력 리스크). "
        f"BIA 노이즈 상수: 가정용 발전극형 ±2-3%p 체지방률, ±1-2kg 제지방(Nickerson 2016, Aandstad 2014). "
        f"하루 2-3kg 스윙은 수분·글리코겐 변동으로 정상.\n"
        f"4. **오늘 추천 (`## 오늘 추천` 섹션, 결정 트리 실행 — '적절히 판단' 금지)**: "
        f"§5에서 산출한 최근 7일 러닝 거리·ACWR·Z1+Z2 비율과 어젯밤 TST를 대입해 아래 우선순위대로 결론 도출.\n"
        f"   ① 어젯밤 TST < 5.0h → 회복 우선(Fullagar 2015: TST<6h = 퍼포먼스 저하)\n"
        f"   ② 최근 7일 러닝 > 30km 또는 ACWR(7일/28일) > 1.3 → 강도 하향(Gabbett 2016 danger zone)\n"
        f"   ③ 최근 러닝 Z1+Z2 < 75% 편중 → Z2 easy 우선(폴라라이즈드 교정, Seiler 2010)\n"
        f"   ④ 그 외 → 오늘 프라임 축 로테이션(러닝/웨이트 교대, Shah 2002 목표간섭 회피)\n"
        f"   **출력 형식(4줄 이내)**: 결론 한 줄 `**오늘: [운동/휴식] 우선** — [근거 수치 한 줄]` "
        f"다음 `A(안전·회복)` / `B(추천)` / `C(공격)` 각 한 줄로 트레이드오프. "
        f"C=완전 휴식은 자율성 옵션으로 항상 유지(오너가 선택할 권리). "
        f"취침 시각은 판단 기준에서 제외(2-4시 취침 정상). "
        f"응원 문구·'일찍 자라' 훈수·격려 이모지 절대 금지.\n"
        f"5. **30일 볼륨 + 러닝 질적 진단 (§4의 결정 트리 입력값)**: "
        f"`mcp__garmin__get_activities`로 30일 실측 조회한 뒤:\n"
        f"   - **볼륨**: 러닝 세션수·거리·주 평균, 웨이트 세션수·주요 리프트, "
        f"롱런(75분+) 월 횟수, LT2(템포/인터벌) 월 횟수\n"
        f"   - **강도 분포 (10K sub-65 정합성)**: 각 러닝 세션의 심박존을 "
        f"`mcp__garmin__get_activity_hr_zones`로 조회 → **Z1+Z2 시간 비율** 산출. "
        f"Z1+Z2 < 75%면 '그레이존(Z3) 편중 — VO2max 정체 원인 후보' 플래그(Seiler polarized 80/20).\n"
        f"   - **§4 입력값 계산**: 최근 7일 러닝 거리와 ACWR(acute 7일 ÷ chronic 28일 평균)을 이 조회 결과에서 직접 산출.\n"
        f"   - 메모리 §11 등 낡은 요약 재사용 금지 — 매일 실측 재조회.\n"
        f"6. **톤 + 마감**: '경보/위험/⚠️' 헤더 금지. 침착·근거 기반. "
        f"밴드 내 변동은 '정체' 대신 누적 진전 프레이밍. "
        f"응원 문구·격려 이모지 금지(감정<직설). 각 섹션 짧게, 액션 별도 강제 금지."
    )
